/**
 * 구글시트 "조교실습코치_일반계약요청"에서 투입 이력을 가져와 engagements에 저장
 * 실행: npx tsx scripts/import-engagements.ts
 */
...
async function main() {
  ...
  if (!sheet) {
    console.error('시트를 찾을 수 없습니다: 조교실습코치_일반계약요청')
    return
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!coachId) {
      ...
      if (startDate && startDate.getFullYear() >= 2026) {
        ...
        console.log(`  코치 자동 생성 (2026 계약): ${name} ${phone || ''} ${email || ''}`)
      } else {
        unmatched++
        unmatchedNames.add(name)
        continue
  ...
  console.log(`\n매칭: ${matched}건, 미매칭: ${unmatched}건, 스킵: ${skipped}건`)
  if (unmatchedNames.size > 0) {
    console.log(`미매칭 코치 (${unmatchedNames.size}명): ${[...unmatchedNames].join(', ')}`)
  }
  ...
  for (const eng of engagements) {
    ...
    // Insert individual work schedules into coach_schedules (always, even if engagement existed)
    for (const sched of eng.schedules) {
      const dateKey = `${sched.date.getFullYear()}-${String(sched.date.getMonth() + 1).padStart(2, '0')}-${String(sched.date.getDate()).padStart(2, '0')}`
  ...
  console.log(`투입 이력 생성: ${created}건, 중복 스킵: ${duplicated}건`)
  console.log(`근로 스케줄 생성: ${schedulesCreated}건`)

  const totalEng = await prisma.engagement.count()
  const totalSched = await prisma.coachSchedule.count()
  console.log(`\n전체 투입 이력: ${totalEng}건, 전체 스케줄: ${totalSched}건`)
}
