...
### 코치 정보 반영 (설문지 28~33행)
- 이승규, 정수진, 김예인, 석은규, 김시은, 양정무 6명 birthDate/workType/affiliation/availabilityDetail 업데이트
- fields/curriculums 반영 (교육 분야 + 가능 분야 + 보유 스킬)
- 중복 필드명 병합: "개발 / 프로그래밍" → "개발/프로그래밍", "자동화 & 업무생산성" → "자동화&업무생산성"
- 마스터 커리큘럼 목록 정비 (UI / UX 추가)

### 지표 대시보드 개선 (`/admin/metrics`)
- 삼전 DS/DX 일정 입력 현황 카드 추가 (프로그레스 바 + 완료/접속만/미확인)
- 일정 제공 비율 카드 추가 (삼전 → 삼전+비삼전 입력완료 / 발송 84명 기준)
- 일별 입력수 추이: SVG 차트 → 히트맵 테이블 전환 (전체/DS/DX, 3일~ 표시)
- 외부 채널 모집 현황: 단일 카드 → 월별 추이 테이블 (10월~, 슬랙 등 + 알바몬 + 카톡방)
- 매니저당 코치 pool: 월별 → 누적 방식 변경 + 전월 대비 delta 표시 (5.2명 → +0.9명)
- 섭외 응답률 설명 추가 (수락+거절)
- 일정 입력률/삼전 현황: 링크 발송 시트 기준 대상자 카운트
- 월 선택 드롭다운 제거 (현재 월 고정)
- 일별 바 차트에 누적 입력률(%) 라인 오버레이 추가 (우측 Y축)
- 히트맵 카드 높이를 인접 카드와 정렬
- 코치 목록 테이블 컬럼 여백 균일화
- 파일 추가 버튼 크기 축소

### 삼전 DS/DX 삼성 대시보드 숨김
- samsung-only 필터에서도 getSamsungExclusions 적용
- env 설정(DS 5월~, DX 4월~)에 따라 삼성 대시보드에서도 숨김 처리
- 찜꽁 노출 차단

### 데이터 수정
- 이헌우 → 이현우 이름 수정 (로컬 + 프로덕션 DB)

### 코치 상세 근무유형 표시
- 코치 상세 페이지(`/coaches/[id]`) 헤더 이름 아래 서브라인에 근무유형 배지 추가
- 목록 테이블과 동일한 색상 체계 (실습코치=보라, 운영조교=틸, 삼전 DS=앰버, 삼전 DX=블루)
- 프로필 탭 파일 행 구조 정리 (근무유형은 헤더로 이동)
...
### 투입 이력 백필
- 프로덕션 DB 83명 코치 기준, 구글시트(`조교실습코치_일반계약요청`) 2022~2026 전체 대조
- 시트에서 workType(F열) + hourlyRate(I열) 매칭
- 39건 + 4건 = 총 43건 백필 완료 (217 → 347건)
- backfill 스크립트: `scripts/backfill-missing-engagements.ts` (dry-run 지원)

### 동기화 개선
- 일반 계약 동기화: 2025-09 이전 스킵 필터 제거 (중복체크만으로 충분)
- 삼전 스케줄 과정명: `삼성전자 SW학부 교육과정` → `(B2B) 삼성전자 SW학부 교육과정_26년`
- 삼전 계약 시트 연동: 새 시트(`운영조교/실습코치 계약요청` 탭)에서 시급/사번/이메일/연락처/매니저/근무유형 보강
- 실습코치 근무유형 자동 합산 (`mergeWorkTypeStrings('삼전 DS', contract.workType)`)

### 대시보드 UI
- 초기화 버튼: 항상 표시, 새로고침 왼쪽 배치
- 레이아웃: 오전/오후/저녁 (좌) | 초기화/새로고침 (우)
- 기본 시간필터: 오전+오후 선택 상태 (약 08:00~18:00 라벨)
- 과정 드롭다운: 날짜 미선택 시에도 항상 표시
- 과정 선택 → 날짜 자동 선택 (캘린더 이동 포함)
- 날짜 클릭 → 드롭다운 초기화, 드롭다운 선택 → 날짜 설정 (상호 배타)
- 과정 선택 상태에서 범위 내 날짜 개별 토글 (범위 밖 클릭 시 초기화)
- 과정 미선택 시 섭외 불가 (회색 버튼 + alert)
- 하루 선택 + 과정 추가 시 시작일/종료일 동일 날짜 셋팅
- 섭외 버튼: "컨택중" → "섭외 요청"으로 변경

### 요청 내역 (마이페이지)
- 과정 수정 모달: 과정명/기간/과정 내용 편집 + 삭제 기능
- Course.description 필드 추가 (DB 마이그레이션)
- 확정 시: 확인 팝업 ("○○ 코치를 확정하시겠습니까? 다른 N명 자동 취소")
- 취소 시: 확인 팝업 ("이 섭외 요청을 취소하시겠습니까?")
- 한 명 확정 → 같은 과정+날짜 다른 수락 코치 자동 취소

### 코치뷰 알림 가독성 개선
- 과정명 상단 굵게, 매니저명 아래 작게, 노트 별도 박스
- enriched 타입에 courseName 추가

### 섭외 에러 수정
- 날짜 파싱 UTC noon 통일 (`toDateOnly()`)
- 7,9,11일 섭외 후 8,10일 추가 요청 시 unique constraint 에러 해결
...
### 헤더 네비게이션 정리
- 마이페이지 진입: 이름 클릭 → "요청 내역" 네비 링크로 변경
- 이름은 오른쪽 끝에 별도 표시, 클릭 시 드롭다운 메뉴(로그아웃)
- 알림 드롭다운 데스크탑 확대 (w-[420px], max-h-[80vh], 모바일은 화면 전체)

### 코치뷰 알림 UX
- 벨 아이콘 → "받은 요청" 버튼으로 변경 (미읽은 수 뱃지)
- 클릭 시 스케줄 박스 아래 알림 섹션으로 smooth 스크롤
- ScoutingAlerts 위치: 스케줄 박스 내부 → 박스 아래로 이동
- 섹션 헤더 "받은 요청 (N)" 추가
- 수락/거절 매니저 알림 클릭 시 마이페이지로 이동 (기존: 코치 상세)

### 마이페이지 UI 정리
- 복사/엑셀/다중선택 체크박스 숨김 (코드 유지, UI만 비표시)
- 코치 칩: 색상 점 → 텍스트 라벨 (컨택중/수락/확정/거절)
- "새 과정 추가" 버튼 제거 (대시보드에서만 생성)
- 회사명 매핑 탭: 마이페이지 → 관리자 페이지로 이동
...
### 메일머지 내보내기 확인
- 코치 개인 토큰 URL(`/coach?token=<accessToken>`)을 포함한 메일 발송 필요
- 기존 `/api/coaches/export` API에 `type: "mail-merge"` 옵션 이미 구현되어 있음 확인
- 이름/이메일/링크(baseUrl + token) 3열 엑셀 출력
- 프로덕션 전체 코치 대상으로 메일머지용 엑셀 다운로드 후 개별 토큰 URL 메일 발송 예정

### 근로시간 열 밀림 버그 수정
- `buildContractRows()`에서 `scheduleLines` 앞에 빈 `""` 하나가 끼어 있어 열이 한 칸씩 밀림
- 복사/엑셀 다운로드 시 근로시간이 이메일 열에 들어가던 문제 해결

### 삭제된 과정 흔적 표시
- `GET /api/courses`에서 soft-delete된 과정도 `deletedCourses` 배열로 반환
- CourseTab 하단에 접이식 "삭제된 과정 (N)" 섹션 — 회색 취소선 + 삭제일 표시

### 미수락 섭외 수정 알림
- 상태머신에 `scouting → scouting` (manager) 전이 추가
- ScoutingTab 팝오버에 "수정 알림 보내기" 버튼
- 기존 T1 만료 → 새 `scouting_request_modified` 알림 발송
- notification-service에 '찜꽁 수정' 타이틀 매핑 추가

### 과정 수정 시 찜꽁 리셋
- 과정 PATCH 시 `accepted`/`confirmed` 찜꽁을 `scouting`으로 자동 리셋
- 코치에게 수정 알림 재발송
- 확인 팝업 분기: 확정 있으면 "확정이 취소되고, 다시 요청 알림이 갑니다", 수락만 있으면 "찜꽁 수락이 취소되고..."

### 섭외 방어로직 (경우의 수 전수 점검)

**EngagementSchedule soft-cancel**
- `cancelledAt DateTime?` 필드 추가 (Prisma 스키마 마이그레이션)
- `cancelEngagementScheduleForScouting` 공유 헬퍼 — 3-tuple (engagementId, coachId, date) 기준
- 3개 호출 지점: 매니저 직접 취소, 과정 삭제 cascade, 과정 수정 리셋
- 활성 스케줄 0건 시 Engagement.status도 `cancelled`로 변경

**알림 갭 수정**
- `accepted → cancelled` 시 코치에게 `engagement_cancelled` 알림 발송 (기존: 없음)
- 과정 삭제 시 confirmed/accepted 코치에게 취소 알림 발송 (기존: 만료만)
- `expireScoutingRequestNotifications`가 `scouting_request_modified` 타입도 함께 만료
- 기존 테스트 `accepted→cancelled = null` → `engagement_cancelled` 기대로 수정

**API 경쟁 조건 방어**
- Coach accept/reject API: `course.deletedAt` 체크 → 409 "과정이 삭제되어 처리할 수 없습니다"
- Manager confirm API: `course.deletedAt` 체크 → 409 "과정이 삭제되어 확정할 수 없습니다"
- Manager UI: 409 에러 시 alert + 찜꽁 목록 새로고침

**UI 방어**
- CourseTab 삭제 메시지: 확정 있으면 "확정된 섭외가 취소됩니다", 없으면 "연결된 찜꽁이 모두 취소됩니다"
- ScoutingAlerts 수락/거절 버튼: readOnly prop 추가

### 코치뷰 매니저 미리보기 모드
- URL에 `?viewer=manager` 추가 시 읽기 전용 모드
- 수락/거절/저장 버튼 숨김, 프로필 수정/알림 벨/활동 중지 비활성화
- 상단 "코치가 보는 화면입니다" 배너, 하단 "매니저 미리보기 모드" 표시

### 코치뷰 UI 미세 조정
- CoachHeader: 패딩 조정
- ScheduleCalendar: 요일 헤더/셀 크기 축소, 간격 조정
- ScheduleSummary: 라운딩/패딩/간격 조정, 빈 상태 중앙정렬

### 문서
- `docs/plans/scouting-defense-matrix.md` — 96셀 상태 전이 매트릭스 (Course 상태 × Scouting 상태 × 8개 액션)
- `docs/plans/scouting-defense-scenarios.md` — 12개 횡단 시나리오
...
**문제**
- 프로덕션/로컬에 코치가 80명이어야 하는데 200+명 표시
- 투입이력 동기화 시 DB에 없는 코치를 자동 생성하는 로직이 원인
- 4/4 동기화 시 과거 이력만 있는 코치 198명(프로덕션 196명)이 자동 생성됨

**조치**
- 로컬 198명 + 프로덕션 196명 soft-delete 처리
- `engagements.ts`: 코치 자동 생성 로직 제거 → DB에 없는 코치는 skip
- 강민혁 코치 접속 로그 5건 삭제 (프로덕션)
- 스테이징/프로덕션 courses 테이블에 `remarks` 컬럼 추가 (과정 생성 오류 해결)
- 스테이징 courses `work_hours` varchar(100) → text 타입 변경

**확인 필요**
- [x] 프로덕션 코치 88명 확인
- [x] 투입이력 동기화 시 코치 자동 생성 안 됨
- [x] 과정 추가 정상 동작
- [ ] 강민혁 코치 "미확인" 상태 확인
...
**구현 완료**
- 헤더/캘린더 분리 레이아웃: 헤더 컨테이너 너비 + rounded-b-2xl, 캘린더 카드 -mt-3 겹침
- 캘린더 셀 축소: text-[13px], rounded-[8px], gap-[3px]
- 요일 헤더: text-[11px] font-medium, 컴팩트 패딩
- 컨테이너 너비 480px → 420px
- 일정 섹션: rounded-2xl, 카드 rounded-[10px], 여백 개선
- 저장 버튼: rounded-[10px], text-[15px], 파란 그림자
- 섭외 알림 헤더 패딩/간격 조정
- ScoutingAlerts readOnly prop 추가

**확인 필요**
- [ ] 헤더→캘린더 전환 부분 디자인 확인
- [ ] 모바일에서 전체 레이아웃 확인
- [ ] 정적 HTML 미리보기 (public/coach-preview.html) 삭제 여부
...
**구현 완료**
- 6개월 월별 trend 라인 차트 + 주간 weeklyTrend 바 차트 제거
- `calcDailyTrend` 함수 추가 — `schedule_access_logs.last_edited_at` / `scoutings.created_at` 일별 GROUP BY (TO_CHAR)
- 당월은 오늘까지만, 지난 달은 전체 일수 표시
- 프론트엔드: 일별 그룹 바 차트 (파란색=일정 입력, 보라색=섭외 발송), X축 5일 간격 레이블
- 외부 구인비율은 MetricSnapshot 수동 입력 → 일별 분해 불가, 카드 월 합산 유지

**확인 필요**
- [ ] /admin/metrics 페이지에서 월 선택 시 일별 바 차트 정상 표시
- [ ] 당월 선택 시 오늘까지만 표시, 지난 달 선택 시 전체 일수 표시
- [ ] 데이터 없는 날은 바 높이 0으로 빈 공간 처리

---

### 프로덕션 DB 마이그레이션 누락 해결

**구현 완료**
- `engagement_schedules.cancelled_at` 컬럼 추가 마이그레이션 생성 + 프로덕션/스테이징 출시
- `metric_snapshots` 테이블 마이그레이션 생성 + 출시 (Sentry COACH-DB-6 해결)
- `courses.remarks` 컬럼 + `work_hours` VARCHAR→TEXT 타입 변경 마이그레이션 출시
- `0_init` migration.sql dotenv 로그 오염 제거
- `add_notifications` ↔ `add_scouting_status` 간 ScoutingStatus enum 참조 순서 수정 (shadow DB용)
- 스테이징 DB 실패 마이그레이션(`cancelled_at`) resolve 처리

**확인 완료**
- [x] 프로덕션 25개 마이그레이션 전체 정상
- [x] 스테이징 25개 마이그레이션 전체 정상
- [x] 로컬 25개 마이그레이션 동기화 완료
...
**구현 완료**
- admin 역할일 때 마이페이지 상단 매니저 선택 드롭다운 추가
- 다른 매니저 선택 시 "열람 모드" 표시
- `/api/courses` — admin일 때 `?managerId=` 파라미터로 다른 매니저 과정 조회
- `/api/engagements/mine` — admin일 때 `?managerId=` 파라미터로 다른 매니저 투입이력 조회
- `/api/scoutings` — 기존 `?managerId=` 파라미터 그대로 활용

**확인 필요**
- [ ] admin 계정으로 다른 매니저 마이페이지 과정/섭외/투입이력 조회
- [ ] 열람 모드에서 쓰기 작업(과정 생성/삭제, 섭외 상태 변경)이 본인 계정 기준으로 동작하는지

---

### 코치 목록 검색 한글 IME 버그 수정

**구현 완료**
- 한글 IME 조합 중(ㅂ→바→박) debounce 발동 → 스켈레톤 전환 → input unmount → 검색어 사라지는 버그 수정
- `compositionstart`/`compositionend` 이벤트로 조합 상태 추적, 조합 중 debounce 건너뜀
- 조합 완료 후 `compositionEnd` 카운터 증가로 useEffect 재실행 → 300ms 후 자동 검색
- 최초 로드 이후 재검색 시 스켈레톤 미표시 (initialLoadDone ref)
- 로딩/빈 결과 화면에도 검색 바 + 필터 슬롯 유지

**확인 필요**
- [ ] 한글 천천히 입력 시 포커스 유지 + 조합 완료 후 자동 검색
- [ ] 영문/숫자 입력 시 기존처럼 즉시 debounce 동작
- [ ] 검색 결과 0건일 때 검색 input 유지 + "검색 조건에 맞는 코치가 없습니다" 표시
...
**구현 완료**
- 같은 과정 알림 그룹핑 (좌측 파란 바 + 배경색 구분)
- 메모를 그룹당 1번만 표시 (개별 카드에서 제거)
- 과정설명 / 전하는 말 라벨 분리 표시 (API에서 마커 파싱 후 별도 필드로 전달)
- 그룹 헤더에 매니저·장소·시급·비고 표시
- 개별 카드는 날짜·시간 + 수락/거절만 compact하게

**확인 필요**
- [ ] 새로 보낸 섭외에서 과정설명 / 전하는 말 분리 표시
- [ ] 기존 마커 없는 메모는 과정설명으로 fallback
...
**구현 완료**
- DashboardContent → courseDescription, message 별도 필드로 전송
- scoutings API → `[과정설명]` / `[전하는 말]` 마커 포맷으로 note에 저장
- coach notifications API → 서버에서 파싱하여 courseDescription, message 분리 전달
- `hasNoteInput` 조건 추가 (note/courseDescription/message 중 하나라도 오면 저장)
...
**구현 완료**
- 근무시간 아래 텍스트 미리보기 추가 (`04-07(월) 09:00 ~ 18:00 (휴게 1H, 총 8H)` 형식)
- 과정 내용 아래 비고 필드 추가 (Course.remarks 컬럼, 코치에게 표시됨)
- EditCourseModal + CourseTab 모두 적용
- API GET/PATCH에 remarks 반영
...
**구현 완료**
- 헤더: 모바일(md 이하)에서 햄버거 메뉴 (드롭다운) + 페이지 이동 시 자동 닫힘
- 알림 팝업: 모바일에서 fixed 양쪽 1rem 여백 배치 (화면 밖 넘침 수정)
- 자동적용 뱃지: inline-block으로 텍스트 안에 배치 (줄 독점 방지)
- 코치 행: flex-wrap + truncate 제거 (모바일 줄바꿈 허용)
- grid-cols-2 → grid-cols-1 sm:grid-cols-2 (8개 파일, 10건)
- grid-cols-4 → grid-cols-2 sm:grid-cols-4 (TimePanel, ScheduleCalendar)
- 코치 페이지 모달/카드: max-w-[400px~480px] → max-w-[calc(100vw-2rem)] sm:max-w-[원래값]
- 대시보드 컨택 모달: max-w-[420px] → 반응형
- CoachListTable 스켈레톤: 반응형 그리드 + overflow-x-auto
...
**구현 완료**
- 7단계 안내 (과정 만들기 → 정보 입력 → 코치 찾기+찜꽁 → 전하는 말 → 코치뷰 → 확정 → 계약)
- 단계별 desc를 배열로 변경하여 여러 문단 지원
- /guide 공개 경로 (미들웨어에서 인증 우회)

---

### 기타

- AI 기초과정 5기 engagement DB에서 삭제
- MetricSnapshot 모델 추가 (지표 스냅샷)
- ApiAccessLog 모델 추가
