...
export default function CoachNotificationBell({ token }: { token: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={handleOpen}
        className="relative p-1 rounded-lg text-gray-400 hover:text-[#333] transition-colors cursor-pointer"
        aria-label="알림"
      >
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-72 max-h-[360px] overflow-y-auto rounded-xl bg-white shadow-lg border border-gray-200 z-50">
          ...
          {loading ? (
            ...
            notifications.slice(0, 20).map((n) => (
              ...
              >
                ...
                <div className="text-[11px] text-gray-600 mt-0.5 line-clamp-2">{n.body}</div>
